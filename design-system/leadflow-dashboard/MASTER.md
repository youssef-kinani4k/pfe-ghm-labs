> **Decision d'implementation (T13, 24 aout 2026).** Le systeme genere propose un fond
> sombre (#0F172A). LeadFlow l'applique en **mode clair** : le dashboard s'exploite a cote de
> la console RabbitMQ et des ERP, tous clairs, et une bascule sombre rendrait l'ensemble
> incoherent d'un ecran a l'autre. Ce qui est effectivement repris : le style Swiss/minimalisme,
> l'echelle d'espacement dense (`--lf-space-*`, 8 -> 32px), Fira Sans sur les titres, Fira Code
> sur les identifiants, et les couleurs de statut (`--lf-succes`, `--lf-attente`, `--lf-echec`)
> definies dans `frontend/src/styles.scss`. La palette Material est `mat.$azure-palette`.
...
**Project:** LeadFlow Dashboard
**Generated:** 2026-08-24 14:29:29
