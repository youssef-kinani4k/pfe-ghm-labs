...
```java
    ...
    void lEchecDeLAttributionRendUnEtatPartielSansResponsable() {
        ...
        assertThatThrownBy(() -> connecteur.sync(lead, cible, apresEchec))
                ...
                .extracting(echec -> ((CrmSyncException) echec).etat())
                .satisfies(etat -> {
                    assertThat(((CrmSyncState) etat).opportunityRef()).isEqualTo("33");
                    assertThat(((CrmSyncState) etat).assigneeRef()).isNull();
                });
    }
```

Si l'accesseur de l'état porté par `CrmSyncException` ne s'appelle pas `etat()`, utiliser le nom réel — le lire dans `crm/model/CrmSyncException.java` avant d'écrire le test.
